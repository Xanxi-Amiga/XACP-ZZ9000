...
int SCR_ModalMessage (char *text)
{
	...
	{ extern volatile unsigned int *shared; shared[SH_MODAL_STAGE] = 1; }
 
// draw a fresh screen
	scr_fullupdate = 0;
	scr_drawdialog = true;
	{ extern volatile unsigned int *shared; shared[SH_MODAL_STAGE] = 2; }
	SCR_UpdateScreen ();
	{ extern volatile unsigned int *shared; shared[SH_MODAL_STAGE] = 3; }
	scr_drawdialog = false;
	
	S_ClearBuffer ();		// so dma doesn't loop current sound
	{ extern volatile unsigned int *shared; shared[SH_MODAL_STAGE] = 4; }

	do
	{
		key_count = -1;		// wait for a key down and up
		Sys_SendKeyEvents ();
		{ extern volatile unsigned int *shared; shared[SH_MODAL_LOOPS]++; shared[SH_MODAL_LASTKEY] = (unsigned int)key_lastpress; }
	} while (key_lastpress != 'y' && key_lastpress != 'n' && key_lastpress != K_ESCAPE);
	{ extern volatile unsigned int *shared; shared[SH_MODAL_STAGE] = 5; }
